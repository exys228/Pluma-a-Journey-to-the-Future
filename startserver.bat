...
:MAIN
java -jar serverstarter-2.3.1-u.jar
GOTO EOF

:CHECK
REM Check if serverstarter JAR is already downloaded
IF NOT EXIST "%cd%\serverstarter-2.3.1-u.jar" (
	ECHO serverstarter binary not found, downloading serverstarter...
	%SYSTEMROOT%\SYSTEM32\bitsadmin.exe /rawreturn /nowrap /transfer starter /dynamic /download /priority foreground https://github.com/exys228/ServerStarter/releases/download/v2.3.1-u/serverstarter-2.3.1-u.jar "%cd%\serverstarter-2.3.1-u.jar"
   GOTO MAIN
) ELSE (
   GOTO MAIN
)
