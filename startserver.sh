...
if [ -f serverstarter-2.3.1-u.jar ]; then
    echo "Skipping download. Using existing serverstarter-2.3.1-u.jar"
    java $JAVA_FLAGS -jar serverstarter-2.3.1-u.jar
    ...
    export URL="https://github.com/exys228/ServerStarter/releases/download/v2.3.1-u/serverstarter-2.3.1-u.jar"
fi
echo $URL
which wget >> /dev/null
if [ $? -eq 0 ]; then
    echo "DEBUG: (wget) Downloading ${URL}"
    wget -O serverstarter-2.3.1-u.jar "${URL}"
else
    which curl >> /dev/null
    if [ $? -eq 0 ]; then
        echo "DEBUG: (curl) Downloading ${URL}"
        curl -o serverstarter-2.3.1-u.jar -L "${URL}"
    else
